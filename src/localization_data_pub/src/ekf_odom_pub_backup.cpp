...
// Biến đánh dấu đã nhận được vị trí ban đầu chưa
bool initialPoseRecieved = false;

// Check encoder is sent or not 
ros::Time lastEncoderDataTime;
bool isRightEncoderDataAvailable = false;
bool isLeftEncoderDataAvailable = false;
...
void Calc_Left(const std_msgs::Int16& leftCount) {
  // ROS_INFO("Received data from left_ticks topic."); 
  // ROS_INFO("Received data from left_ticks topic: %d ticks", leftCount.data); 
  
  lastEncoderDataTime = ros::Time::now();
  isLeftEncoderDataAvailable = true;
...
void Calc_Right(const std_msgs::Int16& rightCount) {
  // ROS_INFO("Received data from right_ticks topic.");
  // ROS_INFO("Received data from left_ticks topic: %d ticks", rightCount.data); 

  lastEncoderDataTime = ros::Time::now();
  isRightEncoderDataAvailable = true;
...
void update_odom() {
  if (!(isRightEncoderDataAvailable && isLeftEncoderDataAvailable)) {
    distanceRight = 0;
    distanceLeft = 0;
  }
...
int main(int argc, char **argv) {
  ...
  while(ros::ok()) {
    // Calculate the time elapsed since the last received encoder data
    ros::Duration elapsedTime = ros::Time::now() - lastEncoderDataTime;

    // Check if the elapsed time exceeds a certain threshold
    const double MAX_TIME_WITHOUT_DATA = 0.5;  // 0.5 seconds without data
    if (elapsedTime.toSec() > MAX_TIME_WITHOUT_DATA) {
        // No encoder data received within the specified time
        isRightEncoderDataAvailable = false;
        isLeftEncoderDataAvailable = false;
    }

    // Print the status of encoder data availability
    ROS_INFO("Right encoder data available: %s", isRightEncoderDataAvailable ? "true" : "false");
    ROS_INFO("Left encoder data available: %s", isLeftEncoderDataAvailable ? "true" : "false");
